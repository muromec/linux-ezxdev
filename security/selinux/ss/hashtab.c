			 ...
			 unsigned int size)
{
	...
	p = (hashtab_t) malloc_sleep(sizeof(hashtab_val_t));
	if (p == NULL)
		return p;
	...
	p->keycmp = keycmp;
	p->htable = (hashtab_ptr_t *) malloc_sleep(sizeof(hashtab_ptr_t) * size);
	if (p->htable == NULL) {
		free(p);
		return NULL;
	}
...
int hashtab_insert(hashtab_t h, hashtab_key_t key, hashtab_datum_t datum)
{
	...
	newnode = (hashtab_ptr_t) malloc_sleep(sizeof(hashtab_node_t));
	if (newnode == NULL)
		return HASHTAB_OVERFLOW;
