 ...
 *	PPC4xx modifications
 * 	MontaVista Software, Inc.
 * 	currently maintained by Armin Kuster akuster@mvista.com
 *
 *	original Author
 * 	MontaVista Software, Inc.
 ...
 *
 *
 *    Module name: ppc4xx_dma.h
 *
 *    Description:
 *      Data structures specific to the IBM PPC4xx on-chip DMA controller
 *      and API.
 *
 *    version 1.1: 02/02 - Armin
 *    added stb03xxx suppor
 *
 *    version 1.2: 02/28/02 - armin
 *    changed config_405gp to a more generic config
 *
 *    version 1.3: 07/18/02 - armin
...
extern int get_dma_status(void);
extern void set_src_addr(int dmanr, phys_addr_t src_addr);
extern void set_dst_addr(int dmanr, phys_addr_t dst_addr);
extern int enable_dma(unsigned int dmanr);
extern void disable_dma(unsigned int dmanr);
extern int set_dma_mode(unsigned int dmanr, unsigned int mode);
extern void set_dma_count(unsigned int dmanr, unsigned int count);
extern int get_dma_residue(unsigned int dmanr);
extern void set_dma_addr(unsigned int dmanr, phys_addr_t addr);
extern void set_dma_addr2(unsigned int dmanr, phys_addr_t src_dma_addr,
			phys_addr_t dst_dma_addr);
extern int enable_dma_interrupt(unsigned int dmanr);
extern int disable_dma_interrupt(unsigned int dmanr);
extern int clr_dma_status(unsigned int dmanr);
extern int map_dma_port(unsigned int dmanr, unsigned int ocp_dma,short dma_chan);
extern int disable_dma_port(unsigned int dmanr, unsigned int ocp_dma,short dma_chan);
