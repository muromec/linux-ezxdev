...
#define DMA_CP            (1<<23)	/* Channel priority */
#define SET_DMA_CP(x)    (((x)&0x1)<<23)
...
#if defined (CONFIG_405XX_DMA)

#define SGL_LIST_SIZE 4096
#define DMA_405_SIZE SGL_LIST_SIZE
...
static __inline__ int get_dma_status(void)
{
	return (mfdcr(DCRN_DMASR));
}


static __inline__ int enable_dma(unsigned int dmanr)
{
 ...
 */
static __inline__ int set_dma_mode(unsigned int dmanr, unsigned int mode)
{
	ppc_dma_ch_t *p_dma_ch = &dma_channels[dmanr];
 ...
 */
static __inline__ void set_dma_addr(unsigned int dmanr, dma_addr_t addr)
{
	ppc_dma_ch_t *p_dma_ch = &dma_channels[dmanr];
#ifdef DEBUG_405DMA
	{
 ...
 */
static __inline__ void 
set_dma_addr2(unsigned int dmanr, dma_addr_t src_dma_addr,
...
enable_dma_interrupt(unsigned int dmanr)
{
	unsigned int control;
	ppc_dma_ch_t *p_dma_ch = &dma_channels[dmanr];
 ...
 */
static __inline__ int 
disable_dma_interrupt(unsigned int dmanr)
{
	unsigned int control;
	ppc_dma_ch_t *p_dma_ch = &dma_channels[dmanr];
 ...
 */
static __inline__ int 
add_dma_sgl(sgl_handle_t handle, dma_addr_t src_addr, dma_addr_t dst_addr,
	unsigned int count)
{
 ...
 */
static __inline__ void enable_dma_sgl(sgl_handle_t handle)
{
	sgl_list_info_t *psgl = (sgl_list_info_t *)handle;
	ppc_dma_ch_t *p_dma_ch;
	uint32_t sg_command;
 ...
 */
static __inline__ void disable_dma_sgl(sgl_handle_t handle)
{
	sgl_list_info_t *psgl = (sgl_list_info_t *)handle;
	uint32_t sg_command;
 ...
 */
static __inline__ int 
get_dma_sgl_residue(sgl_handle_t handle, dma_addr_t *src_addr,
	dma_addr_t *dst_addr)
{
	sgl_list_info_t *psgl = (sgl_list_info_t *)handle;
	ppc_dma_ch_t *p_dma_ch;
 ...
 */
static __inline__ int 
delete_dma_sgl_element(sgl_handle_t handle, dma_addr_t *src_dma_addr,
	dma_addr_t *dst_dma_addr)
{
	sgl_list_info_t *psgl = (sgl_list_info_t *)handle;
