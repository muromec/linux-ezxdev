...
static int
sessions_read_pfs(char *buffer, char **start, linux_off_t offset,
...
/* Returns 0 if dmapi is not busy. */
int
dm_uninit(void)
{
	...
	if( spin_is_locked(&dm_session_lock) ){
		printk(KERN_ERR "dm_uninit: attempt to unload while dm_session_lock is held\n");
		busy=1;
	}
	if( spin_is_locked(&dm_token_lock) ){
		printk(KERN_ERR "dm_uninit: attempt to unload while dm_token_lock is held\n");
		busy=1;
	}
	if( spin_is_locked(&dm_reg_lock) ){
		printk(KERN_ERR "dm_uninit: attempt to unload while dm_reg_lock is held\n");
		busy=1;
	}
	if(dm_sessions_active){
		printk(KERN_ERR "dm_uninit: attempt to unload while there are active sessions\n");
		busy=1;
	}
	if(dm_fsys_cnt){
		printk(KERN_ERR "dm_uninit: attempt to unload while there are filesystems registered.\n");
		busy=1;
	}
