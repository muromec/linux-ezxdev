 ...
 *     Mount record interface for ignoring crc check 
 *
...
#define DJPRINTK(fmt,args...) printk("SCAN.c: %d: " fmt "\n", __LINE__, ## args)
...
int jffs2_scan_medium(struct jffs2_sb_info *c)
{
	...
	/* Check for the very first time mounting after code flashing --Susan*/
	DJPRINTK("empty_blocks(%d), c->nr_erasing_blocks(%d), c->nr_free_blocks(%d)\n", empty_blocks, c->nr_erasing_blocks, c->nr_free_blocks);

	if ( (empty_blocks + 1) == c->nr_blocks )  //The very first time mounting //
	{
		struct list_head *erase_pending_list;

		erase_pending_list = c->erase_pending_list.next;
		while ( erase_pending_list != &c->erase_pending_list )
		{
			size_t retlen;
			struct list_head *tmp;
			struct jffs2_raw_node_ref *marker_ref = NULL;
			struct jffs2_eraseblock *this_jeb = list_entry(erase_pending_list, struct jffs2_eraseblock, list);
			struct kvec vecs[1];
			...
			ret = jffs2_flash_direct_writev(c, vecs, 1, this_jeb->offset, &retlen);
			...
			if (retlen != sizeof(marker)) {
				printk(KERN_WARNING "Short write to newly-erased block at 0x%08x: Wanted %zd, got %zd\n",
				       this_jeb->offset, sizeof(marker), retlen);
				jffs2_free_raw_node_ref(marker_ref);
				return -EIO;
			}
			...
			marker_ref->flash_offset = this_jeb->offset | REF_NORMAL;
			marker_ref->__totlen = c->cleanmarker_size;
				
			this_jeb->first_node = this_jeb->last_node = marker_ref;
				
			this_jeb->free_size = c->sector_size - c->cleanmarker_size;
			this_jeb->used_size = c->cleanmarker_size;
			this_jeb->dirty_size = 0;
			this_jeb->wasted_size = 0;

			tmp = erase_pending_list;
			erase_pending_list = erase_pending_list->next;

			list_del(tmp);  //remove from c->erase_pending_list //
			c->nr_erasing_blocks --;
			empty_blocks --;

			list_add(tmp, &c->free_list);
			c->nr_free_blocks ++;
		}//while loop
	
		DJPRINTK("empty_blocks(%d), c->nr_erasing_blocks(%d), c->nr_free_blocks(%d)\n", empty_blocks, c->nr_erasing_blocks, c->nr_free_blocks);
	}//The very first time mounting //
				  ...
				  struct jffs2_raw_dirent *rd, uint32_t ofs)
{
	...
	ic = jffs2_get_ino_cache(c, je32_to_cpu(rd->pino));
	if (!ic) {
