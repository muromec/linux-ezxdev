 ...
 * Modified by Ru Yi <e5537c@motorola.com> 2005/12/02
 *     Added to count the file real size ocuppied on flash 
 *
...
static int jffs2_add_frag_to_fragtree(struct jffs2_sb_info *c, struct jffs2_inode_info *f, struct jffs2_node_frag *newfrag);
#else
...
#ifdef CONFIG_ARCH_EZXBASE
static void jffs2_obsolete_node_frag(struct jffs2_sb_info *c, struct jffs2_inode_info *f, struct jffs2_node_frag *this) 
#else
...
int jffs2_add_full_dnode_to_inode(struct jffs2_sb_info *c, struct jffs2_inode_info *f, struct jffs2_full_dnode *fn)
{
...
#ifdef CONFIG_ARCH_EZXBASE
	ret = jffs2_add_frag_to_fragtree(c, f, newfrag);
#else
	...
	if (fn->frags) {
		f->f_size += fn->f_size;
		D1(printk(KERN_DEBUG "%s ino #%u add fn->f_size 0x%x to f->f_size 0x%x\n", __FUNCTION__, f->inocache->ino, fn->f_size, f->f_size));
	}
#endif	
...
#ifdef CONFIG_ARCH_EZXBASE
static unsigned long frag_count = 0;
static unsigned long all_frags = 0;
#endif
...
{
#ifdef CONFIG_ARCH_EZXBASE
	struct rb_root *list = &f->fragtree; 
#endif
	...
	if (newfrag->ofs > this->ofs) {
		...
		#ifdef CONFIG_ARCH_EZXBASE
			jffs2_obsolete_node_frag(c, f, this);
		#else
...
#ifdef CONFIG_ARCH_EZXBASE
void jffs2_truncate_fraglist (struct jffs2_sb_info *c, struct jffs2_inode_info *f, uint32_t size)
#else
...
#endif
{
	...
	#ifdef CONFIG_ARCH_EZXBASE
		jffs2_obsolete_node_frag(c, f, frag);
	#else
					...
					struct jffs2_raw_inode *latest_node)
{
	...
	f->dents = fd_list;
#ifdef CONFIG_ARCH_EZXBASE
	if (f->dents && f->dents->f_size) {
		f->f_size += f->dents->f_size;
		D1(printk(KERN_DEBUG "%s ino #%u add f->dents->f_size 0x%x to f->f_size 0x%x\n", __FUNCTION__, f->inocache->ino, f->dents->f_size, f->f_size));
	}
#endif	
