	...
	while (valid_ref) {
		...
		*/
		...
		case JFFS2_NODETYPE_DIRENT:
			...
			fd->f_size = PAD(je32_to_cpu(node.d.totlen));
			D1(printk(KERN_DEBUG "%s: fd->f_size 0x%x dirent.totlen 0x%x\n", __FUNCTION__, fd->f_size, je32_to_cpu(node.d.totlen)));
		#endif
		...
		#ifdef CONFIG_ARCH_EZXBASE
			tn->fn->f_size = PAD(je32_to_cpu(node.i.totlen));
			D1(printk(KERN_DEBUG "%s: tn->fn->f_size 0x%x inode.totlen 0x%x\n", __FUNCTION__, tn->fn->f_size, je32_to_cpu(node.i.totlen)));
		#endif
			tn->fn->raw = ref;
