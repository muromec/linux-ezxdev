		       ...
		       size_t count, loff_t * ppos)
{
	...
	if (state->stop_dma_for_suspend) {
		/* Must block here to stop further DMA activity until
		   the system is resumed */

		DECLARE_WAITQUEUE(wait, current);

		add_wait_queue(&state->suspend_wq, &wait);
repeat:
		set_current_state(TASK_INTERRUPTIBLE);
		if (state->stop_dma_for_suspend && !signal_pending(current)) {
			schedule();
			goto repeat;
		}
		set_current_state(TASK_RUNNING);
		remove_wait_queue(&state->suspend_wq, &wait);
	}
