...
typedef struct {
	...
	struct semaphore sem;		/* prevent races in attach/release */
        /* These DMA variables are used to prepare for suspending this driver
	   (save the registers) prior to sleeping and to restore everything
	   upon wakeup from sleep. The wait queue is used so that audio_read
	   and audio_write can be blocked to stop using the DMA channel while
	   we suspend and resume.
	*/
	int stop_dma_for_suspend;
