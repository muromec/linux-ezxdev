...
*/
	...
	defined(CONFIG_MACH_OMAP_PERSEUS2)
#undef USE_32_BIT
#else
#define USE_32_BIT 1
#endif
...
#ifdef CONFIG_OMAP_INNOVATOR || defined(CONFIG_MACH_OMAP_PERSEUS2)  /* MVL-CEE */
...
static void smc9194_ldm_register(struct net_device *netdev)
{
...
#ifdef CONFIG_ROOT_NFS
   {
	   ...
	   nfs_root_dev = &smc9194_device_ldm;
   }
#endif
}
...
int __init smc_init(struct net_device *dev)
{
	...
	*/
	...
	int base_addr = PERSEUS2_FPGA_ETHR_BASE;
	SET_MODULE_OWNER(dev);
	dev->irq = INT_P2_ETHR;
 	if (smc_probe(dev, base_addr) == 0) {
 		smc9194_ldm_register(dev); /* MVL-CEE */
...
static int __init smc_probe(struct net_device *dev, int ioaddr)
{
	...
	*/
	...
	/* Grab the IRQ */
	retval = request_irq(dev->irq, &smc_interrupt, SA_SHIRQ|SA_SAMPLE_RANDOM
, dev->name, dev);
	if (retval) {
