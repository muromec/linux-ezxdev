/*------------------------------------------------------------------------
 ...
 .	02/20/04  Dmitrij Frasenyak Add DPM support
 ----------------------------------------------------------------------------*/
...
static int nowait = 0;
#ifdef CONFIG_PM
static struct pm_dev *smc_pm_dev = NULL;
static int smc91111_pm_callback(struct pm_dev *, pm_request_t, void *);
#endif
...
#endif /* CONFIG_SYSCTL */
#ifdef CONFIG_DPM
static void smc91111_ldm_register(void);
static void smc91111_ldm_unregister(void);
#endif
#ifdef CONFIG_PM
static int resume_thread(void *);
#endif
...
#ifdef CONFIG_PM
		smc_pm_dev = pm_register(PM_UNKNOWN_DEV, 0, smc91111_pm_callback);
#endif
#ifdef CONFIG_DPM
		smc91111_ldm_register();
#endif
...
#ifdef CONFIG_PM
	init_waitqueue_head (&lp->resume_wq);
	init_completion(&lp->resume_thread_sync);

	lp->resume_thread_exit = 0;
	lp->resume_thread_pid =
		kernel_thread(&resume_thread, (void *)lp,
			      CLONE_FS | CLONE_FILES | CLONE_SIGHAND);
	if (lp->resume_thread_pid < 0) {
		printk(KERN_WARNING "could not start resume thread\n");
		return -ENODEV;
	}
#endif
...
static void smc_timeout(struct net_device *dev)
{
...
#else
	/*
	 * smc_timeout is getting called in an interrupt context on
	 ...
	 */
	PRINTK("Wake up resume_thread\n");

	lp->resume_request = 1;
	wake_up(&lp->resume_wq);
#endif
	dev->trans_start = jiffies;
...
static int smc_close(struct net_device *dev)
{
	...
	if (lp->resume_thread_pid >= 0) {
		lp->resume_thread_exit = 1;
		wake_up(&lp->resume_wq);
		wait_for_completion (&lp->resume_thread_sync);
	}
#endif
...
static void __exit smc_cleanup(void)
{
	...
	unregister_netdev(global_dev);
#ifdef CONFIG_DPM
	smc91111_ldm_unregister();
#endif
#ifdef CONFIG_PM
	if (smc_pm_dev) {
		pm_unregister(smc_pm_dev);
		smc_pm_dev=NULL;
	}
#endif
...
#ifdef CONFIG_PM
int
smc91111_suspend(struct device * dev,  u32 state, u32 level )
{
	int flags;
	...
	case SUSPEND_POWER_DOWN:
		del_timer(&global_dev->watchdog_timer);
...
do_resume(void)
{
	...
	for ( i = 0; i < 6; i += 2 ) {
		...
		address = global_dev->dev_addr[ i + 1 ] << 8 ;
		address  |= global_dev->dev_addr[ i ];
		SMC_outw( address, ioaddr + ADDR0_REG + i );
	}

	{
		struct smc_local *lp 	= (struct smc_local *)global_dev->priv;
		struct net_device *dev = global_dev;
	...
	spin_lock_bh(&global_dev->xmit_lock);
	mod_timer(&global_dev->watchdog_timer, jiffies + global_dev->watchdog_timeo);
	spin_unlock_bh(&global_dev->xmit_lock);

	return;
}

static int
resume_thread(void *x)
{
	struct smc_local *lp 	= (struct smc_local *)global_dev->priv;
	...
	strcpy(current->comm, "smc91111_resume");
	complete(&lp->resume_thread_sync);
	
	for (;;) {
		PRINTK("Going to sleep\n");
		wait_event(lp->resume_wq, lp->resume_request);
		
		PRINTK("Waked up\n");

		if (lp->resume_thread_exit)
			break;
	...
	complete_and_exit(&lp->resume_thread_sync, 0);

	return 0;
}

int
smc91111_resume(struct device * dev, u32 level )
{
	int flags;
	struct smc_local *lp 	= (struct smc_local *)global_dev->priv;
...
static int smc91111_pm_callback(struct pm_dev *dev, pm_request_t rqst, void *data)
{
	switch (rqst){
	case PM_SUSPEND:
		smc91111_suspend(NULL,0,SUSPEND_POWER_DOWN);
		break;
	case PM_RESUME:
		smc91111_resume(NULL,RESUME_POWER_ON);
		break;
	default:
		return -EINVAL;
	}
...
static struct device_driver smc91111_driver_ldm = {
	name:      	"smc91111",
	devclass:  	NULL,
	probe:     	NULL,
	suspend:   	smc91111_suspend,
	resume:    	smc91111_resume,
	scale:	  	NULL,
	remove:    	NULL,
};

static struct device smc91111_device_ldm = {
	name:		"SMC91111 Ethernet Controller",
	bus_id:		"net",
	driver: 	NULL,
...
static void smc91111_ldm_register(void)
{
	...
	plb_driver_register(&smc91111_driver_ldm);
	plb_device_register(&smc91111_device_ldm);
#endif
#ifdef CONFIG_ROOT_NFS
	{
		extern struct device *nfs_root_dev;
		nfs_root_dev = &smc91111_device_ldm;
	}
#endif
}
...
static void smc91111_ldm_unregister(void)
{
	...
	plb_driver_unregister(&smc91111_driver_ldm);
	plb_device_unregister(&smc91111_device_ldm);
#endif
}
