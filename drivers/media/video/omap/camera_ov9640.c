 ...
 * camera_ov9640.c
 *
 * Implementation of Camera for OmniVision OV9640 digital camera.
 *
 ...
 *
 * The register init tables and internal clock calculator (clkrc) were
 * borrowed from Andy Lowe's OV9640 driver for the OMAP2420.
 *
...
static inline void convert_to_BGR24(u16* s, u8* d, int to_user)
{
	unsigned int r,g,b;
	u8 bgr[3];
...
static int ov9640_convert_image(u8* src,
				void* dest,
				int to_user,
				int dest_stride,
				struct v4l2_pix_format* fmt)
{
	...
	int src_BPP = (omap_pixfmt_depth[this->pixfmt]+7) >> 3;
...
{
        int isize;
        unsigned long pixels = fmt->width * fmt->height;

        for (isize = QQCIF; isize < SXGA; isize++) {
                if (omap_image_size[isize + 1].height *
		    omap_image_size[isize + 1].width > pixels) {
			fmt->width = omap_image_size[isize].width;
			fmt->height = omap_image_size[isize].height;
                        return isize;
                }
        }

	fmt->width = omap_image_size[SXGA].width;
	fmt->height = omap_image_size[SXGA].height;
        return SXGA;
}
...
struct camera camera_ov9640 = {
	...
	convert_image: ov9640_convert_image,
