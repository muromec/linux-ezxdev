...
EXPORT_SYMBOL(find_debug_option);
debug_option *find_debug_option (debug_option * options, char *opt2find)
{
	debug_option *op;
	char *sl;
	int r;
...
static void print_options (debug_option * options, char *ps, char *pe)
{
	debug_option *op;
	char *npe;
	for (op = options; NULL != op && NULL != op->name; op++) {
...
static void print_all_options (char *caller_name, debug_option * options)
{
	char opt_prefix[100], *prefix_end;
	prefix_end = sc (opt_prefix, caller_name);
	prefix_end = sc (prefix_end, ":    ");
	print_options (options, opt_prefix, prefix_end);
}

static void set_all_options (debug_option * options, int level)
{
	debug_option *op;
	for (op = options; NULL != op && NULL != op->name; op++) {
		if (NULL == op->sub_table) {
...
static int set_debug_option (char *caller_name, debug_option * options, char *value)
{
	debug_option *op;
	int level;
	char *eq;
...
EXPORT_SYMBOL(scan_debug_options);
