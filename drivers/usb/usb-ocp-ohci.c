...
static int hc_start (ohci_t * ohci)
{
	...
	writel ((roothub_a (ohci) | RH_A_NPS) & ~RH_A_PSM,
		&ohci->regs->roothub.a);
#else
	writel ((roothub_a (ohci) | RH_A_NPS | RH_A_NOCP) & ~RH_A_PSM,
		&ohci->regs->roothub.a);
#endif
...
hc_add_ohci(struct pci_dev *dev, int irq, void *mem_base, unsigned long flags,
	    ohci_t **pohci, const char *name, const char *slot_name)
{
	char buf[8], *bufp = buf;
	ohci_t * ohci;
	int ret;
...
//	ohci->slot_name = slot_name;
	...
	if (hc_start (ohci) < 0) {
		err ("can't start usb-%s", slot_name);
		hc_release_ohci (ohci);
		return -EBUSY;
	}
...
static int __init ohci_hcd_init (void) 
{
	...
	printk("ocp_ohci_init()\n");

	memset((char *)&dev, 0, sizeof(dev));
	strcpy(dev.name, "usb-ocp-ohci");
	strcpy(dev.slot_name, "builtin");

	/*
	 * Request memory resources.
	 */
	if (!request_mem_region(USB0_BASE, USB0_EXTENT, "usb-ohci"))
		return -EBUSY;
	...
	/* Setup the UIC because the boot code doesn't seem to do it. */
	/* internal interrupts are positively asserted, level sensative */
	mtdcr(DCRN_UIC_PR(UIC0), mfdcr(DCRN_UIC_PR(UIC0)) | mask);
	mtdcr(DCRN_UIC_TR(UIC0), mfdcr(DCRN_UIC_TR(UIC0)) & ~mask);

	ret = hc_add_ohci(&dev, USB0_IRQ,
			  (void *)USB0_BASE, 0, &ocp_ohci,
			  "usb-ocp-ohci", "ocp-usb");

	if (ret)
		release_mem_region(USB0_BASE, USB0_EXTENT);
