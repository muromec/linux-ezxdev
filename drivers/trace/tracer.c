...
static int sEventStructSize[TRACE_EV_MAX + 1] =
{
  ...
  sizeof(trace_irq_name)          /* TRACE_IRQ_NAME */,
  sizeof(trace_pid_name)          /* TRACE_PID_NAME */
};
...
TraceEvent:
  ...
  if(ltt_test_bit(pmEventID, &sLogEventDetailsMask))
	...
	break;
      ...
      case TRACE_EV_IRQ_NAME:
	lVarDataBeg = ((trace_irq_name*) pmEventStruct)->irq_name;
	lVarDataLen = ((trace_irq_name*) pmEventStruct)->irq_name_len;
	lDataSize += (uint16_t) lVarDataLen;
	break;

      case TRACE_EV_PID_NAME:
	lVarDataBeg = ((trace_pid_name*) pmEventStruct)->pid_name;
	lVarDataLen = ((trace_pid_name*) pmEventStruct)->pid_name_len;
	lDataSize += (uint16_t) lVarDataLen;
	break;
      }
    }
		 ...
		 unsigned long pmArg)
{
  ...
  switch(pmCmd)
	...
	return -EINVAL;
      ...
      ltt_set_bit(TRACE_EV_IRQ_NAME, &sTracedEvents);
      ltt_set_bit(TRACE_EV_IRQ_NAME, &sLogEventDetailsMask);
      ltt_set_bit(TRACE_EV_PID_NAME, &sTracedEvents);
      ltt_set_bit(TRACE_EV_PID_NAME, &sLogEventDetailsMask);
		...
		struct file*  pmFile)
{
  ...
  /* Is this the daemon opening the device */
  if(lDevMinor == 0)
    {
    ...
    /* This is the daemon firing up */
    sDaemonTaskStruct = current;

    /* Reset the config to defaults, the daemon will customize later */
    tracer_set_default_config();
    }
