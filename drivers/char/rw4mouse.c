/*
 * linux/drivers/char/rw4mouse.c
 * 
 * Copyright 2000-2002 MontaVista Software Inc.
 *      Completed implementation.
 *      Author: MontaVista Software, Inc.  <source@mvista.com>
 * 
 * Based on work from busmouse.c 
 * Copyright (C) 1995 - 1998 Russell King <linux@arm.linux.org.uk>
 *
 * This driver allows buffering of data.
 *
 * Version 1.0 02/03/06 - Armin
 * initial release
 *
 */
...
#define MAX_MS_EVENT   8  /*  this is the buffer size that holds the mouse datai */
...
void busmouse_add_movementbuttons(int mousedev, int dx, int dy, int buttons)
{
	...
	if (changed) {
		...
		mse->buttons = mse->btn[mse->put] = buttons;
		mse->dx[mse->put] = dx;
		mse->dy[mse->put] = dy;
		mse->put ++;
		if(mse->put >= MAX_MS_EVENT) mse->put = 0;
		if(mse->ready < MAX_MS_EVENT) mse->ready ++;
		else {  // we have to lost the last message because of the buffer is full
		// you may also want to enlarge the buffer, but could you bear the possible 
		// delay of most recent mouse event ?
			mse->get ++;
			if(mse->get >= MAX_MS_EVENT) mse->get = 0;
		}
...
repeat:
	...
	dxpos = mse->dx[mse->get];  
	dypos = mse->dy[mse->get];  
	buttons = mse->btn[mse->get];  
