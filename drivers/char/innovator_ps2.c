...
#define FIFO_EMPTY  1
#define CLEAR_INT   (1<<2)
#define MASK_INT    (1<<5)
#define BYTE_AVAIL  (1<<6)
#define RCV_BUSY    (1<<7)
...
{
	int cnt=len;
	while(cnt--&&(inb(PS2_CTRL_REG)&BYTE_AVAIL))
	{
		*(p++)=inb(PS2_DATA_REG);
	}
	return len-cnt-1;
...
#else
		outb(inb(PS2_CTRL_REG)&(~(MASK_INT|CLEAR_INT)),PS2_CTRL_REG);
#endif		
		break;
	}
...
#else
		outb(inb(PS2_CTRL_REG)|MASK_INT,PS2_CTRL_REG);
#endif		
		break;
	}
...
do_hid_tasklet(unsigned long unused)
{
	...
	ret = 50; while(--ret && (inb(PS2_CTRL_REG)&RCV_BUSY)) udelay(100);
#endif
...
#else
	if (request_irq(INT_GPIO_0, (void *) innovator_hid_interrupt,
			SA_SHIRQ, PFX, hid) < 0)
		return -EINVAL;
...
/*???	free_irq(INT_FPGA_ATN, hid); Looks like it does not need to be here */
	if (hid != NULL)
		kfree(hid);
	return;
}
