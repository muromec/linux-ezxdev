 ...
 * 
 * Change history:
 * 2004-Nov-19  -- To provide a new ppp interface for EzX modem  
 * 2005-Apr-4 -- Clear kloc warning
 */
...
#include "modem_ppp_channel.h"
#include "modem_bridge.h"

#define PPP_VERSION	"2.4.2"

#define PPPIOCNEWBRDEVICE    _IOWR('t', 163, int)   /* create ppp bridge device */
#define PPPIOCBINDPEER       _IOWR('t', 164, int)   /* bind with another bridge device */
...
struct ppp {
    ...
    int bridge_index;         /* used by modem bridge */
    int peer_index;
};
...
struct channel {
	...
	struct modem_ppp_channel *chan;	/* public channel data structure */
...
static struct ppp *ppp_create_bridge_device(int * retp);
...
static int ppp_release(struct inode *inode, struct file *file)
{
	...
	if (pf != 0) {
		...
		if (pf->kind == INTERFACE) {
			ppp = PF_TO_PPP(pf);
			if(ppp->bridge_index != -1 && file == ppp->owner)
		     ...
		     unsigned int cmd, unsigned long arg)
{
	...
	if (cmd == PPPIOCDETACH) {
		...
		if (atomic_read(&file->f_count) <= 2) {
			...
			printk(KERN_DEBUG "Modem PPP PPPIOCDETACH file->f_count=%d\n",
	...
	if (pf->kind != INTERFACE) {
		/* can't happen */
		printk(KERN_ERR "Modem PPP: not interface or channel??\n");
		return -EINVAL;
	}
    ...
    case PPPIOCBINDPEER:
         ...
         */
        if( ppp->bridge_index == -1 ) break;
        
        if( get_user(unit, (int*)arg) ) break;
        if( unit < 0 ) break;
    
        err = -ENOENT;
        if( modem_bridge_bind_device(ppp->bridge_index, unit) ) break;
	...
	case PPPIOCSFLAGS:
	    /* 
	     * do not allow set these flags on bridge device
	     */
	    if( ppp->bridge_index != -1 ) break;
	...
	case PPPIOCSMAXCID:
		...
		if (vj == 0) {
			printk(KERN_ERR "Modem PPP: no memory (VJ compressor)\n");
			err = -ENOMEM;
			break;
		}
		ppp_lock(ppp);
				...
				unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
	case PPPIOCNEWBRDEVICE:
	    /* 
         * Create a new bridge device,
         * Note: no need to check if a net device already existed, 
         * because once a net device or bridge device is created, 
         * this ppp device become attached
         */
	    ppp = ppp_create_bridge_device(&err);
	    if( ppp == 0 ) break;
	    file->private_data = &ppp->file;
	    ppp->owner = file;
	    err = -EFAULT;
	    if( put_user(ppp->bridge_index, (int *) arg) )
	        break;
	    err = 0;
	    break;
...
int __init ppp_init(void)
{
	...
	printk(KERN_INFO "Modem PPP generic driver version " PPP_VERSION "\n");
	err = devfs_register_chrdev(PPP_MAJOR, "modemppp", &ppp_device_fops);
	if (err)
		printk(KERN_ERR "failed to register Modem PPP device (%d)\n", err);
	devfs_handle = devfs_register(NULL, "modemppp", DEVFS_FL_DEFAULT,
				      PPP_MAJOR, 0,
...
/*
 * interfaces to modem bridge
 */
static int
ppp_bridge_start_xmit(void* pdata, int len, void * priv)
{
    struct sk_buff *skb;
    int npi, proto;
    unsigned char *pp;
    struct ppp * ppp = (struct ppp *)priv;
    ...
    if( skb_queue_len(&ppp->file.xq) >= 3 )
        modem_bridge_stop_peer( ppp->peer_index );
...
static int 
ppp_bridge_stop_recv(void * priv)
{
	...
	list = &ppp->channels;
	if (list_empty(list)) {
		/* not attached to any physic port, do nothing */
		return 0;
	}

	/* assume not doing multilink, only stop receiving on the first channel */
	list = list->next;
	pch = list_entry(list, struct channel, clist);

	spin_lock_bh(&pch->downl);
    if (pch->chan && pch->chan->ops->flowctl)
        ret = pch->chan->ops->flowctl(pch->chan, 1);
	spin_unlock_bh(&pch->downl);
	return ret;
}

static int
ppp_bridge_wakeup_recv(void * priv)
{
    struct list_head *list;
    struct channel *pch;
    struct ppp *ppp;
    ...
    /* assume not doing multilink, only enable receiving on the first channel */
    list = list->next;
    pch = list_entry(list, struct channel, clist);

    spin_lock_bh(&pch->downl);
    if (pch->chan && pch->chan->ops->flowctl)
        ret = pch->chan->ops->flowctl(pch->chan, 0);
    spin_unlock_bh(&pch->downl);
    return ret;
}

static struct modem_bridge_ops ppp_bridge_ops = {
    ppp_bridge_start_xmit,
    ppp_bridge_stop_recv,
    ppp_bridge_wakeup_recv
};

static struct ppp *
ppp_create_bridge_device(int * retp)
{
    struct ppp *ppp;
    int ret = -ENOMEM;
    int i;
    ...
    ppp->bridge_index = modem_bridge_register_device(ppp, &ppp_bridge_ops);
    if( ppp->bridge_index < 0 )
        goto err;
    ...
    ppp->file.index = cardmap_find_first_free(all_ppp_units);
    ppp->mru = PPP_MRU;
	...
	cardmap_set(&all_ppp_units, ppp->file.index, ppp);
    up(&all_ppp_sem);
    *retp = 0;
...
ppp_xmit_process(struct ppp *ppp)
{
	...
	ppp_xmit_lock(ppp);
    /* Bridge peer may unbind when there is something left to be sent  */
	if (ppp->bridge_index != -1) {
		ppp_push(ppp);
		...
		{
		    if( ppp->peer_index != -1 )
		        modem_bridge_wakeup_peer(ppp->peer_index);
	    }
	}
	ppp_xmit_unlock(ppp);
}
	...
	if (proto < 0x8000) {
		...
		*skb_push(skb, 2) = 1;
		if (ppp->pass_filter.filter
			...
			if (ppp->debug & 1) {
				printk(KERN_DEBUG "Modem PPP: outbound frame not passed\n");
				kfree_skb(skb);
				return;
			}
		}
...
ppp_do_recv(struct ppp *ppp, struct sk_buff *skb, struct channel *pch)
{
	...
	if (ppp->dev != 0 || (ppp->bridge_index != -1 && ppp->peer_index != -1) )
...
void
modem_ppp_input(struct modem_ppp_channel *chan, struct sk_buff *skb)
{
...
void
modem_ppp_input_error(struct modem_ppp_channel *chan, int code)
{
	struct channel *pch = chan->ppp;
	struct sk_buff *skb;
...
ppp_receive_nonmp_frame(struct ppp *ppp, struct sk_buff *skb)
{
	...
	case PPP_VJC_COMP:
		...
		if (skb_tailroom(skb) < 124) {
			...
			if (ns == 0) {
				printk(KERN_ERR "Modem PPP: no memory (VJ decomp)\n");
				goto err;
			}
			skb_reserve(ns, 2);
			memcpy(skb_put(ns, skb->len), skb->data, skb->len);
		...
		if (len <= 0) {
			printk(KERN_DEBUG "Modem PPP: VJ decompression error\n");
			goto err;
		}
		len += 2;
		if (len > skb->len)
	...
	case PPP_VJC_UNCOMP:
		...
		if (slhc_remember(ppp->vj, skb->data + 2, skb->len - 2) <= 0) {
			printk(KERN_ERR "Modem PPP: VJ uncompressed error\n");
			goto err;
		}
		proto = PPP_IP;
		break;
		...
		*skb_push(skb, 2) = 0;
		if (ppp->pass_filter.filter
		    && sk_run_filter(skb, ppp->pass_filter.filter,
				     ppp->pass_filter.len) == 0) {
			if (ppp->debug & 1)
				printk(KERN_DEBUG "Modem PPP: inbound frame not passed\n");
			kfree_skb(skb);
			return;
		}
		if (!(ppp->active_filter.filter
        ...
        if( ppp->bridge_index != -1 && ppp->peer_index != -1 )
        {
			skb_pull(skb, 2);	/* chop off protocol */
			skb->protocol = htons(npindex_to_ethertype[npi]);
			skb->mac.raw = skb->data;
			modem_bridge_send2peer( ppp->peer_index, skb, 0 );
        }
        else
        {
			kfree_skb(skb);
...
#if 0
		else if ((ppp->dev->flags & IFF_UP) == 0
...
ppp_decompress_frame(struct ppp *ppp, struct sk_buff *skb)
{
	...
	if (proto == PPP_COMP) {
		...
		if (ns == 0) {
			printk(KERN_ERR "Modem ppp_decompress_frame: no memory\n");
			goto err;
		}
 ...
 */
int
modem_ppp_register_channel(struct modem_ppp_channel *chan)
{
	struct channel *pch;
 ...
 */
int modem_ppp_channel_index(struct modem_ppp_channel *chan)
{
	struct channel *pch = chan->ppp;
 ...
 */
int modem_ppp_unit_number(struct modem_ppp_channel *chan)
{
	struct channel *pch = chan->ppp;
	int unit = -1;
 ...
 */
void
modem_ppp_unregister_channel(struct modem_ppp_channel *chan)
{
	struct channel *pch = chan->ppp;
 ...
 */
void
modem_ppp_output_wakeup(struct modem_ppp_channel *chan)
{
	struct channel *pch = chan->ppp;
...
ppp_create_interface(int unit, int *retp)
{
	...
	ppp->bridge_index = ppp->peer_index = -1;
...
static void ppp_shutdown_interface(struct ppp *ppp)
{
	...
	down(&all_ppp_sem);
	ppp_lock(ppp);
    bridge_index = ppp->bridge_index;
    peer_index = ppp->peer_index;
    ppp->bridge_index = ppp->peer_index = -1;
	ppp_unlock(ppp);
    ...
    {
        if( peer_index != -1 )
            modem_bridge_unbind_device(bridge_index, peer_index);
        modem_bridge_unregister_device(bridge_index);        
    }
...
static void ppp_destroy_interface(struct ppp *ppp)
{
	if (!ppp->file.dead || ppp->n_channels) {
		/* "can't happen" */
		printk(KERN_ERR "Modem ppp: destroying ppp struct %p but dead=%d "
		       "n_channels=%d !\n", ppp, ppp->file.dead,
		       ppp->n_channels);
		return;
	}
...
static void ppp_destroy_channel(struct channel *pch)
{
	if (!pch->file.dead) {
		/* "can't happen" */
		printk(KERN_ERR "Modem ppp: destroying undead channel %p !\n",
		       pch);
		return;
	}
...
static void __exit ppp_cleanup(void)
{
	...
	if (atomic_read(&ppp_unit_count) || atomic_read(&channel_count))
		printk(KERN_ERR "Modem PPP: removing module but units remain!\n");
	cardmap_destroy(&all_ppp_units);
	if (devfs_unregister_chrdev(PPP_MAJOR, "modemppp") != 0)
		printk(KERN_ERR "Modem PPP: failed to unregister PPP device\n");
...
int modem_ppp_idle_time( struct modem_ppp_channel * chan, struct ppp_idle * idle )
{
    struct channel *pch;
...
EXPORT_SYMBOL(modem_ppp_register_channel);
EXPORT_SYMBOL(modem_ppp_unregister_channel);
EXPORT_SYMBOL(modem_ppp_channel_index);
EXPORT_SYMBOL(modem_ppp_unit_number);
EXPORT_SYMBOL(modem_ppp_input);
EXPORT_SYMBOL(modem_ppp_input_error);
EXPORT_SYMBOL(modem_ppp_output_wakeup);
...
EXPORT_SYMBOL(modem_ppp_idle_time);
