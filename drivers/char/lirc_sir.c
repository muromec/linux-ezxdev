...
static devfs_handle_t           devfs_lirc;
#endif
...
#ifdef CONFIG_DEVFS_FS
	devfs_lirc = devfs_register( NULL, "lirc", DEVFS_FL_DEFAULT,
				       major, 0,
...
static int init_hardware(void)
{
	...
	if (machine_is_h3xxx()) {
		...
		assign_h3600_egpio(IPAQ_EGPIO_IR_ON, 1);
	}
#endif
	sr.hscr0=Ser2HSCR0;
...
#ifdef CONFIG_SA1100_H3XXX
	if (machine_is_h3xxx()) {
		assign_h3600_egpio(IPAQ_EGPIO_IR_ON, 0);
	}
#endif
#else
	/* turn off interrupts */
