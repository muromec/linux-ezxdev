...
static int __init rtc_init(void)
{
	...
	unsigned int rtc_size;   

#if defined(CONFIG_ARCH_OMAP1510)
	rtc_base = OMAP1510_RTC_BASE;
	rtc_size = OMAP1510_RTC_SIZE;
#elif defined(CONFIG_ARCH_OMAP1610)
	rtc_base = OMAP1610_RTC_BASE;
	rtc_size = OMAP1610_RTC_SIZE;
#elif defined(CONFIG_ARCH_OMAP730)
	rtc_base = OMAP730_RTC_BASE;
	rtc_size = OMAP730_RTC_SIZE;
#else
#error ERROR : Architecture not supported by this driver.
#endif

	if (!request_region(rtc_base, rtc_size,
			    rtc_dev.name)) {
		printk(KERN_ERR "%s: RTC I/O port %d is not free.\n",
		       rtc_dev.name, rtc_base);
		return -EIO;
	}
	...
	if (request_irq(INT_RTC_TIMER, rtc_interrupt, SA_INTERRUPT,
		...
		release_region(rtc_base, rtc_size);
		return -EIO;
	}
...
static void __exit rtc_exit (void)
{
	...
	release_region (OMAP730_RTC_BASE, OMAP730_RTC_SIZE);
#elif defined(CONFIG_ARCH_OMAP1610)
	release_region (OMAP1610_RTC_BASE, OMAP1610_RTC_SIZE);
#else
