 ...
 *
 * Modem bridge async serial channel driver for Linux.
 * Reused from ppp async. Two kinds of changes are made:
 * 1) add a flow control call back function for modem generic layer
 * 2) change to use export functions from modem generic layer
 *
...
struct asyncppp {
	...
	struct modem_ppp_channel chan;	/* interface to generic ppp layer */
...
MODULE_PARM_DESC(flag_time, "modem_ppp_async: interval between flagged packets (in clock ticks)");
...
static int ppp_async_send(struct modem_ppp_channel *chan, struct sk_buff *skb);
static int ppp_async_flowctl(struct modem_ppp_channel *chan, int onoff);
static int ppp_async_push(struct asyncppp *ap);
static void ppp_async_flush_output(struct asyncppp *ap);
static void ppp_async_input(struct asyncppp *ap, const unsigned char *buf,
			    char *flags, int count);
static int ppp_async_ioctl(struct modem_ppp_channel *chan, unsigned int cmd,
			   unsigned long arg);
static void async_lcp_peek(struct asyncppp *ap, unsigned char *data,
			   int len, int inbound);

static struct modem_ppp_channel_ops async_ops = {
	ppp_async_send,
	ppp_async_ioctl,
	ppp_async_flowctl
};
...
ppp_asynctty_open(struct tty_struct *tty)
{
	...
	ap->chan.mtu = PPP_MRU;
	err = modem_ppp_register_channel(&ap->chan);
	if (err)
		goto out_free;
 ...
 * finish before we can call modem_ppp_unregister_channel and free
...
ppp_asynctty_close(struct tty_struct *tty)
{
	 ...
	 * Note that modem_ppp_unregister_channel ensures that no calls to
	...
	modem_ppp_unregister_channel(&ap->chan);
	if (ap->rpkt != 0)
		kfree_skb(ap->rpkt);
	if (ap->tpkt != 0)
		kfree_skb(ap->tpkt);
		   ...
		   unsigned int cmd, unsigned long arg)
{
	...
	case PPPIOCGCHAN:
		...
		if (ap == 0)
			break;
		err = -EFAULT;
		if (put_user(modem_ppp_channel_index(&ap->chan), (int *) arg))
			break;
		err = 0;
		break;
	...
	case PPPIOCGUNIT:
		...
		if (ap == 0)
			break;
		err = -EFAULT;
		if (put_user(modem_ppp_unit_number(&ap->chan), (int *) arg))
			break;
		err = 0;
		break;
	...
	case FIONREAD:
		...
		break;
	case PPPIOCGIDLE:
        if( 0 != modem_ppp_idle_time( (&ap->chan), &idle ) )
            break;
    ...
    case MODEMESCAPESTATUS:
		if (put_user(ap->got_esc, (int *) arg))
			break;
		err = 0;
        break;
 ...
 */
static ssize_t 
ppp_asynctty_chars_in_buffer(struct tty_struct *tty)
{
    struct asyncppp *ap = ap_get(tty);
    ssize_t ret = 0;
		  ...
		  char *flags, int count)
{
    ...
    /* 
     * Lower layer may call this function even though we just throttle it,
     * so we do not call unthrottle here.
     */
/*
...
ppp_asynctty_wakeup(struct tty_struct *tty)
{
	...
	if (ppp_async_push(ap))
		modem_ppp_output_wakeup(&ap->chan);
	ap_put(ap);
}
...
static struct tty_ldisc ppp_ldisc = {
	...
	chars_in_buffer: ppp_asynctty_chars_in_buffer,
...
static int __init
modem_ppp_async_init(void)
{
	int err;

	err = tty_register_ldisc(N_MODEM_PPP, &ppp_ldisc);
	if (err != 0)
		printk(KERN_ERR "Modem_PPP_async: error %d registering line disc.\n",
		       err);
	return err;
}
 ...
 */
static int
ppp_async_ioctl(struct modem_ppp_channel *chan, unsigned int cmd, unsigned long arg)
{
...
EXPORT_SYMBOL(modem_ppp_crc16_table);
#define fcstab	modem_ppp_crc16_table		/* for PPP_FCS macro */
...
/*
 * function used by generic layer to turn on/off flow control
 */
static int ppp_async_flowctl(struct modem_ppp_channel *chan, int bflowCtlOn)
{
	struct asyncppp *ap = chan->private;
    ...
    if( bflowCtlOn )
    {
        ...
        /* turn flow control on , stop receiving from lower layer */
    ...
    else
    {
        ...
        /* turn flow control off, can receive from lower layer */
        if (test_and_clear_bit(TTY_THROTTLED, &tty->flags)
            && tty->driver.unthrottle)
        {
            tty->driver.unthrottle(tty);
 ...
 * If the packet was not accepted, we will call modem_ppp_output_wakeup
 * at some later time.
 */
static int
ppp_async_send(struct modem_ppp_channel *chan, struct sk_buff *skb)
{
	struct asyncppp *ap = chan->private;
...
process_input_packet(struct asyncppp *ap)
{
	...
	modem_ppp_input(&ap->chan, skb);
	return;

 err:
	kfree_skb(skb);
	modem_ppp_input_error(&ap->chan, code);
}
		...
		char *flags, int count)
{
    ...
    if ( count <= (MAX_ESC_CHAR_COUNT - ap->esc_char_count) ) {
        if ( (ap->esc_char_count > 0) 
            || ( (ap->esc_char_count == 0) && ((jiffies - ap->last_recv_for_esc) >= 10) ) ) {
            ...
            if (k == count) {
                esc_char_recv = 1;
                if (ap->esc_char_count == MAX_ESC_CHAR_COUNT) {
                    /* OK, we got escape +++ */
                    ap->got_esc = 1;
                    printk("\nModem ppp async: got escape +++!\n");
                    if( kill_sl(tty->session, SIGHUP, 1) != 0 ) {
                        printk("\nModem ppp async: fail to send SIGHUP to modem engine!\n");
                    }
    ...
    if (!esc_char_recv) {
        ap->esc_char_count = 0;
        ap->last_recv_for_esc = jiffies;
    }

    /* Work-around for BP can not handler escape +++ correctly */
    if(ap->got_esc)
    { 
        printk("PPP work-around for BP escape bug\n");
        return;
    }
    /*~END of work-around*/
...
static void __exit modem_ppp_async_cleanup(void)
{
	if (tty_register_ldisc(N_MODEM_PPP, NULL) != 0)
		printk(KERN_ERR "failed to unregister Modem PPP line discipline\n");
}

module_init(modem_ppp_async_init);
module_exit(modem_ppp_async_cleanup);
MODULE_LICENSE("GPL");
