...
static int
pcmcia_set_reg_value(int sock, PXA_REG_t reg)
{
	unsigned int clock, speed;

	clock = get_memclk_frequency_10khz();

	/* how to get the pccard_mem_map or pccard_io_map? */
	
	switch (reg) {
	case MCMEM:
		speed = pxa_pcmcia_socket[sock].speed_mem;
		if(speed <= 0) {
	...
	case MCATT:
		if (sock == 0) {
			MCATT0 = ((pxa_mcxx_setup(PXA_PCMCIA_ATTR_MEM_ACCESS,
						  clock)
				   & MCXX_SETUP_MASK) << MCXX_SETUP_SHIFT)
				 | ((pxa_mcxx_asst(PXA_PCMCIA_ATTR_MEM_ACCESS,
						   clock)
				     & MCXX_ASST_MASK) << MCXX_ASST_SHIFT)
				 | ((pxa_mcxx_hold(PXA_PCMCIA_ATTR_MEM_ACCESS,
						   clock)
				     & MCXX_HOLD_MASK) << MCXX_HOLD_SHIFT);
		} else {
			MCATT1 = ((pxa_mcxx_setup(PXA_PCMCIA_ATTR_MEM_ACCESS,
						  clock)
				   & MCXX_SETUP_MASK) << MCXX_SETUP_SHIFT)
				 | ((pxa_mcxx_asst(PXA_PCMCIA_ATTR_MEM_ACCESS,
						   clock)
	...
	case MCIO:
		speed = pxa_pcmcia_socket[sock].speed_io;
		if (sock == 0) {
	...
	default:
		printk(KERN_ERR "pcmcia_set_reg_value: Invalid reg %d\n",
		       reg);
		return -1;
	}
...
static int pxa_pcmcia_ldm_suspend(struct device * dev, u32 state, u32 level);
static int pxa_pcmcia_ldm_resume(struct device * dev, u32 level);
static int pxa_pcmcia_ldm_scale(struct bus_op_point * op, u32 level);

static struct device_driver pxa_pcmcia_driver_ldm = {
	name:      	"pxapcmcia",
	devclass:  	NULL,
	probe:     	NULL,
	suspend:   	pxa_pcmcia_ldm_suspend,
	resume:    	pxa_pcmcia_ldm_resume,
	scale:	  	pxa_pcmcia_ldm_scale,
	remove:    	NULL,
};

static struct device pxa_pcmcia_device_ldm = {
	name:		"PXA PCMCIA Controller",
	bus_id:		"pcmcia",
	driver:		NULL,
	power_state:	DPM_POWER_ON,
};

static void pxa_pcmcia_ldm_register(void)
{
	extern void pxasys_driver_register(struct device_driver *driver);
	extern void pxasys_device_register(struct device *device);

	pxasys_driver_register(&pxa_pcmcia_driver_ldm);
	pxasys_device_register(&pxa_pcmcia_device_ldm);
}

static void pxa_pcmcia_ldm_unregister(void)
{
	...
	pxasys_driver_unregister(&pxa_pcmcia_driver_ldm);
	pxasys_device_unregister(&pxa_pcmcia_device_ldm);
}
...
static struct saved_regs regs;

static void restore_regs(struct saved_regs *regs)
{
	MECR = regs->saved_mecr;
	MCMEM0 = regs->saved_mcmem0;
	MCMEM1 = regs->saved_mcmem1;
	MCATT0 = regs->saved_mcatt0;
	MCATT1 = regs->saved_mcatt1;
	MCIO0 = regs->saved_mcio0;
	MCIO1 = regs->saved_mcio1;
}

static void save_regs(struct saved_regs *regs)
{
	regs->saved_mecr = MECR;
	regs->saved_mcmem0 = MCMEM0;
	regs->saved_mcmem1 = MCMEM1;
	regs->saved_mcatt0 = MCATT0;
	regs->saved_mcatt1 = MCATT1;
	regs->saved_mcio0 = MCIO0;
	regs->saved_mcio1 = MCIO1;
}

static int pxa_pcmcia_ldm_resume(struct device * dev, u32 level)
{
	extern int pcmcia_dpm_resume_card(int);
	...
	case RESUME_POWER_ON:
		/* restore registers */
		restore_regs(&regs);
		/* enable the GPIOs with low-level function */
		if (pcmcia_low_level->mask_irqs) {
			pcmcia_low_level->mask_irqs(0);
		}
		/* enable the PCMCIA controller */
		MECR |= GPIO_bit(1);
		/* resume the cards */
		pcmcia_dpm_resume_card(0);
		pcmcia_dpm_resume_card(1);
		break;
	}
...
static int pxa_pcmcia_ldm_suspend(struct device * dev, u32 state, u32 level)
{
	extern int pcmcia_dpm_suspend_card(int);
	...
	case SUSPEND_POWER_DOWN:
		/* disable the low-level sockets */
		pcmcia_dpm_suspend_card(0);
		pcmcia_dpm_suspend_card(1);
		
		/* save the registers */
		save_regs(&regs);
		/* disable the GPIOs with low-level function */
		if (pcmcia_low_level->mask_irqs) {
			pcmcia_low_level->mask_irqs(1);
		}
		/* disable the PCMCIA controller */
		MECR &= ~GPIO_bit(1);
		break;
	}
...
static int pxa_pcmcia_ldm_scale(struct bus_op_point * op, u32 level)
{
	...
	for (i = 0; i < pxa_pcmcia_socket_count; i++) {
		pcmcia_set_reg_value(i, MCMEM);
		pcmcia_set_reg_value(i, MCATT);
		pcmcia_set_reg_value(i, MCIO);
	}
...
static int __init pxa_pcmcia_driver_init(void){
  ...
  set_GPIO_mode(GPIO85_nPCE_1_MD);
  ...
  if(machine_is_lubbock()){
    ...
    pcmcia_low_level=&mainstone_pcmcia_ops;
#endif
...
static int pxa_pcmcia_set_io_map(unsigned int sock,
  ...
  if(map->flags&MAP_ACTIVE){
    ...
    pcmcia_set_reg_value(sock, MCIO);
...
static int pxa_pcmcia_set_mem_map(unsigned int sock,
  ...
  if(map->flags&MAP_ACTIVE){
    ...
    if(map->flags&MAP_ATTRIB){
	    ...
	    pcmcia_set_reg_value(sock, MCATT);
    } else {
	    pxa_pcmcia_socket[sock].speed_mem=speed;
	    pcmcia_set_reg_value(sock, MCMEM);
    }
    DEBUG(4, "%s(): FAST%u %lx  BSM%u %lx  BSA%u %lx  BSIO%u %lx\n",
	  __FUNCTION__, sock, MECR_FAST_GET(mecr, sock), sock,
	  MECR_BSM_GET(mecr, sock), sock, MECR_BSA_GET(mecr, sock), 
