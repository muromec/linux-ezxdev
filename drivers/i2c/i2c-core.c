	...
	for (addr = 0x00; addr <= 0x7f; addr++) {
		...
		for (i = 0;
			...
			if (addr == address_data->normal_i2c[i]) {
				found = 1;
				DEB2(printk(KERN_DEBUG "i2c-core.o: found normal i2c entry for adapter %d, " "addr %02x", adap_id,addr));
printk(KERN_DEBUG "i2c-core.o: found normal i2c entry for adapter %d, " "addr %02x", adap_id,addr);
			}
		}
...
#endif
#ifdef CONFIG_I2C_DBMX2
	extern int i2c_dbmx2_init(void);
#endif
...
#ifdef CONFIG_I2C_DBMX2 /*algorithm/adapter i2c driver for Motorola Dragonball i.MX21*/
	i2c_dbmx2_init();
#endif
