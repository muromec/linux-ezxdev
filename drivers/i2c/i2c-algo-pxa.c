			...
			int count, int last)
{
        ...
        for (i = 0; i <= count; i++) {
                if (i!=0){
                        ...
                        if ((i == count) && last) {
				adap->transfer(adap, last, I2C_RECEIVE, 0);
			}else{
				adap->transfer(adap, 0, I2C_RECEIVE, 1);
                        }

			timeout = adap->wait_for_interrupt(adap, I2C_RECEIVE);
                ...
                if (i) {
                        buf[i - 1] = adap->read_byte(adap);
                } else {
                        adap->read_byte(adap); /* dummy read */
                }
        }
        return (i - 1);
}
                         ...
                         int count, int last)
{
        ...
        for (wrcount=0; wrcount<count; ++wrcount) {
                ...
                adap->write_byte(adap, buf[wrcount]);
                if ((wrcount==(count-1)) && last) {
			adap->transfer(adap, last, I2C_TRANSMIT, 0);
		}else{
			adap->transfer(adap, 0, I2C_TRANSMIT, 1);
                }
...
int i2c_pxa_add_bus(struct i2c_adapter *i2c_adap)
{
	...
	if (pxa_scan) {
		...
		for (i = 0x02; i < 0xff; i+=2) {
			...
			if (adap->wait_bus_not_busy(adap)) {
				printk(KERN_INFO "I2C: scanning bus %s - TIMEOUTed.\n",
						i2c_adap->name);
                                return -EIO;
			}
			adap->write_byte(adap, i);
			adap->start(adap);
			adap->transfer(adap, 0, I2C_TRANSMIT, 0);

			if ((adap->wait_for_interrupt(adap, I2C_TRANSMIT) != BUS_ERROR)) {
				printk("(%02x)",i>>1);
				adap->abort(adap);
			} else {
//				printk(".");
