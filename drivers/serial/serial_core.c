...
static int serial_ldm_suspend(struct device *dev, u32 mode, u32 level);
static int serial_ldm_resume(struct device *dev, u32 level);

static struct device_driver serial_driver_ldm = {
	.name = "serial_core",
	.suspend = serial_ldm_suspend,
	.resume = serial_ldm_resume,
};
...
#elif CONFIG_ARCH_MX2ADS
		serial_ldm_suspend(info->state->ldm, 0, SUSPEND_POWER_DOWN);
#else
		if (info->ops->pm)
...
#elif CONFIG_ARCH_MX2ADS
		serial_ldm_resume(info->state->ldm, RESUME_POWER_ON);
#else
		if (info->ops->pm)
...
extern void mx2ads_console_init(void);
...
#ifdef CONFIG_ARCH_MX2ADS
static int
serial_ldm_suspend(struct device *dev, u32 mode, u32 level)
{
	struct uart_state *state = dev->platform_data;
	struct uart_port *port = state->port;
	struct uart_ops *ops = port->ops;
	int running = state->info &&
		      state->info->flags & ASYNC_INITIALIZED;
...
static int
serial_ldm_resume(struct device *dev, u32 level)
{
	struct uart_state *state = dev->platform_data;
	struct uart_port *port = state->port;
	struct uart_ops *ops = port->ops;
...
uart_setup_port(struct uart_driver *drv, struct uart_state *state)
{
	...
	state->cons = drv->cons;
#endif
	state->ldm = (struct device *)kmalloc(sizeof(struct device),GFP_KERNEL);
	memset(state->ldm, 0, sizeof(struct device));
	sprintf(state->ldm->name,"UART %d",state->port->line);
	sprintf(state->ldm->bus_id,"uart_%d",state->port->line);
	state->ldm->power_state = DPM_POWER_ON;
	state->ldm->driver = &serial_driver_ldm;
	state->ldm->platform_data = state;
	mx21_ldm_device_register(state->ldm);
#endif
	...
	if (!drv->cons || port->line != drv->cons->index) {
		serial_ldm_suspend(state->ldm, 0, SUSPEND_POWER_DOWN);
	}
#endif
}
...
#ifdef CONFIG_ARCH_MX2ADS
		if(state->ldm) {
			mx21_ldm_device_unregister(state->ldm);
			kfree(state->ldm);
		}
#endif
...
{
#ifdef CONFIG_ARCH_MX2ADS
	mx21_ldm_driver_register(&serial_driver_ldm);
#endif
	return 0;
}
...
{
#ifdef CONFIG_ARCH_MX2ADS
	mx21_ldm_driver_unregister(&serial_driver_ldm);
#endif
