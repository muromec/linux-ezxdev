 ...
 *
 *  $Id: serial_8250.c,v 1.18 2001/12/21 22:20:30 rmk Exp $
 */
...
#else
                /* Work around for bad hardware use of SA-1110 CS2 for I/O access */
                {
                        unsigned int value, skid_dly;
                        value = readb((unsigned long)port->membase
					+ (offset<<port->regshift));
                        skid_dly = SBI_SKID;
                        udelay(1);
                        return (value);
...
#if	defined(CONFIG_SA1100_PFS168)
		serial8250_ports[i].membase	= old_serial_port[i].iomem_base;
		serial8250_ports[i].regshift	= old_serial_port[i].iomem_reg_shift;
		serial8250_ports[i].iotype	= old_serial_port[i].io_type;
#endif
	}
}
