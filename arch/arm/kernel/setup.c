...
//a17400: add ezx platform special tag
extern u32 power_up_reason;
static int __init parse_tag_ezx(const struct tag *tag)
{
  ...
  if (APP_REBOOT_FLAG == *(unsigned long *)(phys_to_virt(FLAG_ADDR))) {
	printk(KERN_WARNING "user need reboot to full service mode\n");
	power_up_reason = 0;
  }
  *(unsigned long *)(phys_to_virt(FLAG_ADDR)) = DEFAULT_FLAG;
#endif

  return 0;
}

__tagtable(ATAG_EZX,parse_tag_ezx);
...
#elif defined(CONFIG_ARCH_OMAP1610) || defined(CONFIG_ARCH_OMAP730)
