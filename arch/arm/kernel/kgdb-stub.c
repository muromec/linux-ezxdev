...
static inline u8
ebin2mem_escaped(const char *buf)
{
	return (*buf == 0x7d) ? *(++buf) ^ 0x20 : *buf;
}
...
static char *ebin2mem(const char *buf, char *mem, int count)
{
	...
	if ((count == 2) && (((long)mem & 1) == 0)) {
		u16 tmp_s = ebin2mem_escaped(buf++) << 8 |
			    ebin2mem_escaped(buf++);
		*(u16 *)mem = be16_to_cpu(tmp_s);
		mem += 2;
	} else if ((count == 4) && (((long)mem & 3) == 0)) {
		u32 tmp_l = ebin2mem_escaped(buf++) << 24 |
			    ebin2mem_escaped(buf++) << 16 |
			    ebin2mem_escaped(buf++) << 8 |
			    ebin2mem_escaped(buf++);
		*(u32 *)mem = be32_to_cpu(tmp_l);
		mem += 4;
	} else {
		for (; count > 0; count--, buf++) {
			*mem++ = ebin2mem_escaped(buf);
		}
	}
	return mem;
}
