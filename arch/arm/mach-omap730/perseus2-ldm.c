/*
 * Perseus2 ldm routines.
 *
 * Based on original 1610 H2 code - author unknown but appreciated!
 *
...
tipb_bus_match(struct device *dev, struct device_driver *drv)
{
	...
	if (strncmp(dev->bus_id, "uart", 4) == 0)
        ...
        if (strcmp(dev->bus_id, "I2C") == 0)
	        return (strcmp(drv->name, "i2c-adap-omap730") == 0);
        if (strcmp(dev->bus_id, "net") == 0)
	        return (strcmp(drv->name, "smc91x") == 0);
        if (strcmp(dev->bus_id, "IrDA") == 0)
	        return (strcmp(drv->name, "omap730-IrDA") == 0);
	if (strcmp(dev->bus_id, "MMC_SD") == 0)
	...
	if (strcmp(dev->bus_id, "keypad") == 0)
		return (strcmp(drv->name, "perseus2-keypad") == 0);
	if (strcmp(dev->bus_id, "Camera") == 0)
	        return (strcmp(drv->name, "omap-camera") == 0);
	if (strcmp(dev->bus_id, "I2S_Audio") == 0)
		return (strcmp(drv->name, "omap730-syren") == 0);
...
void __init perseus2_ldm_setup(void)
{
