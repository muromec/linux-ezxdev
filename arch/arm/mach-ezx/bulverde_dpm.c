 ...
 *
 * 2005-May-11  copy from arch/arm/mach-pxa/bulverde_dpm.c and porting to EZX platform,  Zhuang Xiaofan
...
#define DPM_FSCALER_EXITING_13M    (1 << 6)
...
static void exit_13m_mode(struct dpm_regs *regs, int fscaler_flags)
{
	int tmp_cccr, tmp_ccsr;

	if (!(fscaler_flags & DPM_FSCALER_EXITING_13M)){
		return;
	}
	...
	*/
	...
	/* Step 1 */
	tmp_cccr = CCCR;
	tmp_cccr |= (CCCR_PPDIS_BIT_ON | CCCR_CPDIS_BIT_ON | \
		     CCCR_PLL_EARLY_EN_BIT_ON);
	CCCR = tmp_cccr;

	/* Step 2 */
	tmp_cccr = CCCR;
	if ((tmp_cccr & 0xC4000000) != 0xC4000000) { /* xPDIS, pll_early_en */
		printk("DPM: Warning: xPDIS or PLL_EARLY_EN not on\n");
	}
	...
	/* Step 3 */
	{
		...
		for (i=0; i<999999 /* arbitrary big value to prevent
				      looping forever */; i++) {
			tmp_ccsr = CCSR;
			if ((tmp_ccsr & 0x30000000) == 0x30000000) {
				/* They are both on */
				break;
			}
		}
	...
	regs->cccr &= ~(CCCR_CPDIS_BIT_ON | CCCR_PPDIS_BIT_ON); /* Not ON */
...
static void mainstone_fscaler(struct dpm_regs *regs)
{
	...
	if(dpm_fscaler_flags & DPM_FSCALER_EXITING_13M)
		exit_13m_mode(regs, dpm_fscaler_flags);
...
compute_fscaler(struct dpm_md_opt *cur, struct dpm_md_opt *new)
{
	...
	if (cur->m13_mode)
		dpm_fscaler_flags |= DPM_FSCALER_EXITING_13M;
...
*/
	...
	if (opt->l == 1) {
		...
		opt->regs.cccr |= (CCCR_PPDIS_BIT_ON | CCCR_CPDIS_BIT_ON);
	}
}
...
dpm_bulverde_init_opt(struct dpm_opt *opt)
{
	...
	int cccra      = opt->pp[DPM_MD_CCCRA]; /* Alternate setting
						  for the MEM clock */
	int plls_on    = opt->pp[DPM_MD_PLLS_ON];
	...
	if (l == PLL_L_MIN) {
		...
		if (plls_on != 0) {
			printk(KERN_WARNING
			       "DPM: When L=1 (13M Mode), PLLs must be OFF (%d)\n",
			       plls_on);
			return -EINVAL;
		}
...
bulverde_get_current_info(struct dpm_md_opt *opt)
{
	...
	/* are the PLLs on? */
	xpdis = ((opt->regs.cccr >> 30) & 0x3);
	if ((xpdis != 0) && (xpdis != 3)) {
		printk("DPM: cpdis and ppdis are not in sync!\n");
	}
	opt->plls_enabled = (xpdis == 0);
				   ...
				   struct dpm_md_opt *new)
{
	...
	if (new->cccra == -1)
          new->cccra = cur->cccra;
	if (new->plls_enabled == -1)
          new->plls_enabled = cur->plls_enabled;
	if (new->sleep_mode == -1)
...
valid_opt(struct constraints *constraints, struct dpm_opt *opt)
{
	...
	for (i = 0;
		...
		case DPM_MD_CCCRA:
			...
			break;
		case DPM_MD_PLLS_ON:
			if ((md_opt->plls_enabled != -1) &&
			    ((param->min > md_opt->plls_enabled) ||
			     (param->max < md_opt->plls_enabled))) {
				dpm_trace(DPM_TRACE_CONSTRAINT_ASSERTED,
					  param->id, md_opt->plls_enabled,
					  param->min, param->max);
				return 0;
			}
			break;
	...
	list_for_each_prev(head, &global_device_list) {
		...
		/* This is a special check to make sure that
		   ...
		   re-enabled. And that is not good. */
			...
			printk("DPM: Constraints deasserted for %s, "
...
dpm_proc_print_opt(char *buf, struct dpm_opt *opt)
{
        ...
        len += p5d(buf + len, md_opt->plls_enabled);
