	       ...
	       unsigned long pfn, unsigned long flags)
{
	...
	pgprot = __pgprot(L_PTE_PRESENT | L_PTE_YOUNG | L_PTE_DIRTY | flags);
	do {
 ...
 */
static void * __local_ioremap(unsigned long phys_addr, size_t size,
			      unsigned long flags)
{
	void * addr;
	...
	if (remap_area_pages(VMALLOC_VMADDR(addr),
			     phys_addr >> PAGE_SHIFT, size, flags)) {
		vfree(addr);
		return NULL;
	}
...
void * __ioremap(unsigned long phys_addr, size_t size,
		 unsigned long flags)
{
	return __local_ioremap(phys_addr, size, flags | L_PTE_WRITE);
}

void * __ioremap_readonly(unsigned long phys_addr, size_t size,
			  unsigned long flags)
{
	return __local_ioremap(phys_addr, size, flags & ~L_PTE_WRITE);
}
