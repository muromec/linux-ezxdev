 ...
 * we can (re-)use ARM userland aligment fault fixups for Thumb.
 *
 ...
 * gdb/sim/arm/thumbemu.c. I managed to marginally improve upon that
 * implementation WRT reduced code size/increased speed. But this is
 * basically just a code reduction of same to convert only Thumb ld/st
 * instruction forms to equivalent ARM forms. So credit goes to the gdb
 * folks for the clever algorithm. Aint the GPL just grand!  8 )
 *
 * NOTE: If for some reason we're passed an non-ld/st Thumb instruction to
 *	 decode, we return 0xdeadc0de. This should never happen under normal
 *	 circumstances but if it does, you've got other problems to deal with
...
int do_alignment(unsigned long addr, int error_code, struct pt_regs *regs)
{
	...
	instr = thumb_mode(regs) ?
		thumb2arm((tinstr = *(u16 *)(instrptr & ~1))) :
	...
	if (ai_usermode & 1)
	{
		if (thumb_mode(regs)) {
			printk("Alignment trap: %s (%d) PC=0x%08lxT Instr=0x%04x "
			       "(0x%08lx) Address=0x%08lx Code 0x%02x\n",
			       current->comm, current->pid, instrptr, tinstr, instr,
			       addr, error_code);
		} else {
