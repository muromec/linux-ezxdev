...
/*
 * There is much much uglyness in the world.  IXP1200 requires that
 * all PCI config accesses be dword aligned/dword at a time. In 
 * addition, the IXP1200 is always used in BE mode.  This requires
 * us to do some LE<->BE conversion when doing config accesses
 * as the built-in host bridge does not to this. 
 *
 * However...the host bridge itself, when accessed from the internal
 * PCI_CSR address is already big endian, so we have to not do any
 * conversion.  This makes the code rather ugly and makes me want
 * to cry.
 */
...
{
	u32 *addr = ixp1200_base_address(dev, where);

	/*
	 * Check to see if reading the data of the IXP itself.
	 * If so, than we don't do endian swapping as we're
	 * accessing it internally. Otherwise we do endian swapping,
	 * which will turn into a no-op when running in LE mode.
	 * (Though it's highly suggested not to run the IXP in LE mode)
	 *
	 * The same applies for all other config operations
	 */
	if(!addr)
		*value = 0xff;
	else if(((u32)addr & PCI_CSR_BASE) != PCI_CSR_BASE) 
		*value = (u8)((__le32_to_cpu(*addr) >> ((where % 0x04) * 8)) & 0xff);
	else 
		*value = (u8)((*addr >> ((where % 0x04) * 8)) & 0xff);
...
ixp1200_read_config_word(struct pci_dev *dev, int where, u16 *value)
{
	...
	if(!addr)
		...
		*value = (u16)((__le32_to_cpu(*addr) >> ((where % 0x04) * 8)) & 0xffff);
	else
		*value = (u16)(((*addr) >> ((where % 0x04) * 8)) & 0xffff);
...
ixp1200_write_config_byte(struct pci_dev *dev, int where, u8 value)
{
	...
	mask = ~(0xff << ((where % 0x4) * 8));

	if(((u32)addr & PCI_CSR_BASE) != PCI_CSR_BASE) {
		temp = (u32)value << ((where % 0x4) * 8);
		temp |= (__le32_to_cpu(*addr)) & mask;
		*(u32 *)addr = __cpu_to_le32(value);
	} else {
		temp = value << ((where % 0x4) * 8);
		*(u32*)addr = (*(u32*)addr & mask) | temp;
	}
...
ixp1200_write_config_word(struct pci_dev *dev, int where, u16 value)
{
	...
	mask = ~(0xffff << ((where % 0x4) * 8));

	if(((u32)addr & PCI_CSR_BASE) != PCI_CSR_BASE) {
		temp = __cpu_to_le16(value) << ((where % 0x4) * 8);
		temp |= (__le32_to_cpu(*addr)) & mask;
		*addr = __cpu_to_le32(value);
	} else {
...
void __init ixp1200_pci_init(struct arm_pci_sysdata *sysdata)
{
	...
	*CSR_SA_CONTROL |= IXP1200_SA_CONTROL_PNR; /*Negate PCI reset*/
	 ...
	 * the "ixphost" parameter to force the bus scan. If not
	 * forced and the PCF pin doesn't say we're the host, we
	 * will assume we are running as an agent.
	 */
	if(!ixp_host)
